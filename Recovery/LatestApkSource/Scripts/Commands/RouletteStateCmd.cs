...
public class RouletteStateCmd : ICommand
{
	...
	public RouletteStateCmd(RouletteManager rouletteManager, AudioManager audioManager, RouletteState rouletteState)
	{
	...
	public void Execute()
	{
		...
		case RouletteState.Pause:
			if (_rouletteManager.lastState != RouletteState.Game)
			{
				return;
			}
			Pause();
	...
	private void Cashout()
	{
		...
		_rouletteManager.gameActive.Value = false;
		_rouletteManager.tableActive.Value = false;
		_rouletteManager.OnCashoutRequired.OnNext(value: true);
	}
