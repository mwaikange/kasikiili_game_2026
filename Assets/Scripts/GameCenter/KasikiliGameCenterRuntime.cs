...
namespace Kasikili.GameCenter
{
    ...
    public sealed class GameCenterController : MonoBehaviour
    {
        ...
        private static readonly Color Ink = new Color32(25, 25, 28, 255);
        private static readonly Color Charcoal = new Color32(76, 78, 87, 255);
        private static readonly Color Green = new Color32(107, 160, 72, 255);
        private static readonly Color DarkGreen = new Color32(3, 53, 33, 255);
        private static readonly Color Lavender = new Color32(211, 207, 255, 255);
        ...
        public void Build()
        {
            ...
            GameObject background = AddImage("Background", screen, 0f, 0f, 375f, 812f, White, false);
            ...
            AddProfileButton(screen, 18f, 34f, 72f, OpenDrawer);
            GameObject balanceCard = AddButton("BalanceCard", screen, 3f, 131f, 322f, 99f, Charcoal, null);
            balanceText = AddText("Balance", balanceCard.transform, FormatBalance(), 18f, 0f, 290f, 99f, 28,
            ...
            for (int i = 0; i < games.Count; i++)
            {
                int row = i / 2;
                int column = i % 2;
                AddGameCard(games[i], 20f + column * 196f, 282f + row * 268f);
            }
        ...
        {
            return "BALANCE : N$ " + GameCenterWallet.MainBalance.ToString("0.00", CultureInfo.InvariantCulture);
        }

        private void AddGameCard(GameCatalogItem game, float x, float y)
        {
            GameObject card = AddButton("Game_" + game.id, screen, x, y, 160f, 224f, Color.clear,
                delegate { SelectGame(game); });

            GameObject icon = AddImage("Icon", card.transform, 10f, 0f, 140f, 140f, Lavender, true, circleSprite);
            Sprite wheel = FindLoadedSprite("wheel");
            ...
            {
                AddAvatarArt(icon.transform, 20f, 20f, 100f);
            }

            AddText("Name", card.transform, game.displayName, 0f, 154f, 160f, 56f, 18, Ink,
                TextAnchor.UpperCenter, FontStyle.Bold);
            if (!game.available)
                AddText("Availability", card.transform, "COMING SOON", 20f, 207f, 120f, 17f, 11, Charcoal,
        ...
        {
            scrim = AddButton("DrawerScrim", screen, 0f, 0f, 375f, 812f, new Color(0f, 0f, 0f, 0.28f), CloseDrawer);
            scrim.SetActive(false);

            drawer = Rect("NavigationDrawer", screen, 0f, 0f, 285f, 812f);
            AddImage("DrawerBackground", drawer, 0f, 0f, 285f, 812f, Green, false);
            AddProfileButton(drawer, 84f, 48f, 88f, CloseDrawer);
            AddText("Title", drawer, "Kasikili Game\nCenter", 20f, 144f, 245f, 86f, 30, Color.black,
                TextAnchor.MiddleCenter, FontStyle.Bold);
            ...
            {
                GameObject item = AddButton("Menu_" + labels[i], drawer, 0f, 244f + i * 83f, 270f, 67f,
                    Charcoal, actions[i]);
                AddText("Label", item.transform, labels[i], 12f, 0f, 246f, 67f, 25, Color.white,
                    TextAnchor.MiddleLeft, FontStyle.Normal);
            }

            drawer.anchoredPosition = new Vector2(-285f, 0f);
        }
        ...
        private void BuildTransferPanel()
        {
            ...
            GameObject transfer = AddButton("Transfer", body.transform, 65f, 380f, 205f, 58f, Green, delegate
            {
                decimal value;
                string error;
        ...
        private void OpenCashOut()
        {
            ...
            GameObject action = AddButton("CashOut", body.transform, 65f, 330f, 205f, 58f, Green, delegate
            {
                decimal value;
                string error;
                if (!decimal.TryParse(amount.text, NumberStyles.Number, CultureInfo.InvariantCulture, out value))
        ...
        private void OpenRewards()
        {
            ...
            GameObject copy = AddButton("Copy", body.transform, 65f, 318f, 205f, 56f, Green, delegate
            {
        ...
        private GameObject OpenModal(string title)
        {
            ...
            modal = AddImage("ReceptionPanel", screen, 0f, 0f, 375f, 812f, White, true);
            GameObject header = AddImage("Header", modal.transform, 0f, 0f, 375f, 82f, DarkGreen, false);
            ...
            AddText("Title", header.transform, title, 68f, 0f, 290f, 82f, 24, Color.white,
                TextAnchor.MiddleCenter, FontStyle.Bold);
            return AddImage("PanelBody", modal.transform, 20f, 112f, 335f, 520f, DarkGreen, false);
        }
        ...
        {
            GameObject button = AddButton("ProfileMenuButton", parent, x, y, size, size, Lavender, onClick, circleSprite);
            AddAvatarArt(button.transform, size * 0.16f, size * 0.16f, size * 0.68f);
        }

        private void AddAvatarArt(Transform parent, float x, float y, float size)
        {
            AddImage("Head", parent, x + size * 0.27f, y + size * 0.05f, size * 0.46f, size * 0.46f,
                new Color32(120, 73, 44, 255), false, circleSprite);
            AddImage("Cap", parent, x + size * 0.18f, y, size * 0.64f, size * 0.26f,
                new Color32(168, 46, 29, 255), false, circleSprite);
            AddImage("Body", parent, x + size * 0.08f, y + size * 0.54f, size * 0.84f, size * 0.38f,
                new Color32(151, 232, 206, 255), false, circleSprite);
        }

        private InputField AddInput(Transform parent, string name, float x, float y, float width, float height, string hint)
        {
            GameObject box = AddImage(name, parent, x, y, width, height, White, true);
